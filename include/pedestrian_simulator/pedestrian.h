...
public:
  ...
  GaussianPedestrian(const Waypoint& start, const Waypoint& end, int seed_mp) : Pedestrian(start), seed_mp_(seed_mp)
  {
    ...
    B = Eigen::Vector2d(std::cos(angle), std::sin(angle));
  }

  virtual void Update() override
  {
    Eigen::Vector2d process_noise_realization = random_generator_->BivariateGaussian(
        Eigen::Vector2d(0., 0.), CONFIG.process_noise_[0], CONFIG.process_noise_[1], angle);
...
public:
  ...
  BinomialPedestrian(const Waypoint& start, int seed_mp) : Pedestrian(start), seed_mp_(seed_mp)
  {
  ...
  virtual void Update() override
  {
    ...
    Eigen::Vector2d process_noise_realization = random_generator_->BivariateGaussian(
        Eigen::Vector2d(0., 0.), CONFIG.process_noise_[0], CONFIG.process_noise_[1], 0.);
...
//     virtual void Reset()
//     {
//         cur_seed_++; // We increase the seed after every simulation, to keep the behavior the same during each
//         simulation random_generator_.reset(new Helpers::RandomGenerator(cur_seed_));
...
public:
  ...
  RandomGaussianPedestrian(double random_x_min, double random_x_max, double random_y_min, double random_y_max,
                           int seed_mp)
  ...
  virtual void Reset()
  {
    if (CONFIG.repeat_experiment_ == -1)
      cur_seed_++;  // We increase the seed after every simulation, to keep the behavior the same during each simulation
    else
      cur_seed_ = seed_mp_ * 10000 + CONFIG.seed_ + CONFIG.repeat_experiment_ + 1;
...
{
public:
  WaypointPedestrian(const Waypoint& start) : Pedestrian(start)
  {
  }
