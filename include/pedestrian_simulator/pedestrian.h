  ...
  virtual void Update() override
  {
    ...
    switch (state)
    {
      case PedState::STRAIGHT:
        ...
        // Transition
        if ((counter % 4 == 0) && (random_generator_->Double() <= p))  // Do this only once every 4 times (to match the
                                                                       // integrator step)
    ...
    UpdatePosition(twist_.linear.x + process_noise_realization(0), twist_.linear.y + process_noise_realization(1));
  }
